...
def forget_user(key: str | None, tool_context: ToolContext) -> str:
    ...
    if not key:
        # 清除所有记忆
        keys_to_remove = [k for k in tool_context.state if k.startswith("memory_")]
        for k in keys_to_remove:
            tool_context.state[k] = ""  # 设为空字符串，持久化时会覆盖
        return f"已清除所有记忆（共 {len(keys_to_remove)} 条）"

    state_key = f"memory_{key}"
    if state_key in tool_context.state:
        tool_context.state[state_key] = ""  # 设为空字符串标记删除
        return f"已清除记忆: {key}"
    return f"未找到记忆: {key}"


all_tools = [remember_user, forget_user]
